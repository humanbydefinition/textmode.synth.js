...
`}}let E=null;function D(e,t,r){const a=r??E;if(a)try{a(e,t)}catch{}}function O(e){if(e===void 0)return"undefined";if(e===null)return"null";if(typeof e=="number"){if(Number.isNaN(e))return"NaN";if(!Number.isFinite(e))return e>0?"Infinity":"-Infinity"}if(Array.isArray(e)){const t=e.findIndex(r=>typeof r!="number"||!Number.isFinite(r));if(t>=0)return`array with invalid element at index ${t}: ${O(e[t])}`}return String(e)}function q(e,t,r){return a=>{let n;try{n=e(a)}catch(s){return D(s,t,a.onError),r}if((o=n)==null||!(typeof o=="number"?Number.isFinite(o):Array.isArray(o)&&o.length>0&&o.every(s=>typeof s=="number"&&Number.isFinite(s))))return D(new Error(`[textmode.synth.js] Invalid dynamic parameter value for "${t}": ${O(n)}`),t,a.onError),r;var o;return n}}class Z{_uniforms=new Map;_dynamicUpdaters=new Map;processArgument(t,r,a){if((function(n){return Array.isArray(n)&&n.length>0&&typeof n[0]=="number"})(t)){const n=`${a}_${r.name}`,o={name:n,type:r.type,value:r.default??0,isDynamic:!0},s=q(c=>(function(l,f){const h=l._speed??1,y=l._smooth??0;let v=f.time*h*(f.bpm/60)+(l._offset??0);if(y!==0){const C=l._ease??j.linear,_=v-y/2,m=l[Math.floor(U(_,l.length))],i=l[Math.floor(U(_+1,l.length))];return C(Math.min(U(_,1)/y,1))*(i-m)+m}return l[Math.floor(U(v,l.length))]})(t,c),n,o.value);return this._uniforms.set(n,o),this._dynamicUpdaters.set(n,s),{glslValue:n,uniform:o,updater:s}}if(typeof t=="function"){const n=`${a}_${r.name}`,o={name:n,type:r.type,value:r.default??0,isDynamic:!0},s=q(t,n,o.value);return this._uniforms.set(n,o),this._dynamicUpdaters.set(n,s),{glslValue:n,uniform:o,updater:s}}if(typeof t=="number")return{glslValue:x(t)};if(Array.isArray(t)&&typeof t[0]=="number"){const n=t;if(n.length===2)return{glslValue:`vec2(${x(n[0])}, ${x(n[1])})`};if(n.length===3)return{glslValue:`vec3(${x(n[0])}, ${x(n[1])}, ${x(n[2])})`};if(n.length===4)return{glslValue:`vec4(${x(n[0])}, ${x(n[1])}, ${x(n[2])}, ${x(n[3])})`}}return this.processDefault(r)}processDefault(t){const r=t.default;return typeof r=="number"?{glslValue:x(r)}:Array.isArray(r)?{glslValue:`vec${r.length}(${r.map(x).join(", ")})`}:{glslValue:"0.0"}}getUniforms(){return new Map(this._uniforms)}getDynamicUpdaters(){return new Map(this._dynamicUpdaters)}clear(){this._uniforms.clear(),this._dynamicUpdaters.clear()}}function x(e){const t=e.toString();return t.includes(".")?t:t+".0"}function X(e){return new ee().compile(e)}class ee{_uniformManager=new Z;_feedbackTracker=new Q;_externalLayerManager=new H;_codeGenerator=new J;_glslFunctions=new Set;_mainCode=[];_varCounter=0;_currentTarget="main";_usesCharSource=!1;compile(t){this._reset();const r=this._compileChain(t,"main","vec4(1.0, 1.0, 1.0, 1.0)","v_uv","main");let a=r.charVar;t.charSource&&(a=this._compileCharSource(t));let n=r.colorVar;t.colorSource&&(n=this._compileChain(t.colorSource,"charColor","vec4(1.0, 1.0, 1.0, 1.0)","v_uv","charColor").colorVar);let o="vec4(0.0, 0.0, 0.0, 0.0)";t.cellColorSource&&(o=this._compileChain(t.cellColorSource,"cellColor","vec4(0.0, 0.0, 0.0, 0.0)","v_uv","cellColor").colorVar);const s=(function(f,h,y){return f?`
	// Character output from generator
	vec4 charOutput = ${h};`:`
	// Derive character from color luminance
	float lum = _luminance(${y}.rgb);
	int charIdx = int(lum * 255.0);
	vec4 charOutput = vec4(float(charIdx % 256) / 255.0, float(charIdx / 256) / 255.0, 0.0, 0.0);`})(!!a,a??"vec4(0.0)",r.colorVar),c=this._feedbackTracker.getUsage();return{fragmentSource:(function(f){const{uniforms:h,glslFunctions:y,mainCode:v,charOutputCode:C,primaryColorVar:_,cellColorVar:m,charMapping:i,usesFeedback:u,usesCharFeedback:d,usesCellColorFeedback:b,usesCharSource:p,externalLayers:M}=f,B=Array.from(h.values()).map(F=>`uniform ${F.type} ${F.name};`).join(`
...
uniform int u_charMapSize;`,$=`
	...
	charOutput.g = float(mappedCharIdx / 256) / 255.0;`);const k=[];u&&k.push("uniform sampler2D prevCharColorBuffer;"),d&&k.push("uniform sampler2D prevCharBuffer;"),b&&k.push("uniform sampler2D prevCellColorBuffer;");const oe=k.join(`
`),se=p?"uniform float u_charSourceCount;":"",V=[];if(M)for(const[,F]of M)F.usesChar&&V.push(`uniform sampler2D ${F.uniformPrefix}_char;`),F.usesCharColor&&V.push(`uniform sampler2D ${F.uniformPrefix}_primary;`),F.usesCellColor&&V.push(`uniform sampler2D ${F.uniformPrefix}_cell;`);return`#version 300 es
