...
vec4 ${`src_${e}`}(vec2 _st) {
	return texture(${Q[e]}, fract(_st));
}
`}}class H{_uniforms=new Map;_dynamicUpdaters=new Map;processArgument(e,t,a){if((function(n){return Array.isArray(n)&&n.length>0&&typeof n[0]=="number"})(e)){const n=`${a}_${t.name}`,s={name:n,type:t.type,value:t.default??0,isDynamic:!0},o=c=>(function(l,d){const m=l._speed??1,h=l._smooth??0;let v=d.time*m*(d.bpm/60)+(l._offset??0);if(h!==0){const x=l._ease??Y.linear,p=v-h/2,y=l[Math.floor(R(p,l.length))],i=l[Math.floor(R(p+1,l.length))];return x(Math.min(R(p,1)/h,1))*(i-y)+y}return l[Math.floor(R(v,l.length))]})(e,c);return this._uniforms.set(n,s),this._dynamicUpdaters.set(n,o),{glslValue:n,uniform:s,updater:o}}if(typeof e=="function"){const n=`${a}_${t.name}`,s={name:n,type:t.type,value:t.default??0,isDynamic:!0};return this._uniforms.set(n,s),this._dynamicUpdaters.set(n,e),{glslValue:n,uniform:s,updater:e}}if(typeof e=="number")return{glslValue:b(e)};if(Array.isArray(e)&&typeof e[0]=="number"){const n=e;if(n.length===2)return{glslValue:`vec2(${b(n[0])}, ${b(n[1])})`};if(n.length===3)return{glslValue:`vec3(${b(n[0])}, ${b(n[1])}, ${b(n[2])})`};if(n.length===4)return{glslValue:`vec4(${b(n[0])}, ${b(n[1])}, ${b(n[2])}, ${b(n[3])})`}}return this.processDefault(t)}processDefault(e){const t=e.default;return typeof t=="number"?{glslValue:b(t)}:Array.isArray(t)?{glslValue:`vec${t.length}(${t.map(b).join(", ")})`}:{glslValue:"0.0"}}getUniforms(){return new Map(this._uniforms)}getDynamicUpdaters(){return new Map(this._dynamicUpdaters)}clear(){this._uniforms.clear(),this._dynamicUpdaters.clear()}}function b(r){const e=r.toString();return e.includes(".")?e:e+".0"}function U(r){return new W().compile(r)}class W{_uniformManager=new H;_feedbackTracker=new G;_externalLayerManager=new K;_codeGenerator=new N;_glslFunctions=new Set;_mainCode=[];_varCounter=0;_currentTarget="main";compile(e){this._reset();const t=this._compileChain(e,"main","vec4(1.0, 1.0, 1.0, 1.0)","v_uv","main");let a=t.charVar;e.charSource&&(a=this._compileCharSource(e));let n=t.colorVar;e.colorSource&&(n=this._compileChain(e.colorSource,"charColor","vec4(1.0, 1.0, 1.0, 1.0)","v_uv","charColor").colorVar);let s="vec4(0.0, 0.0, 0.0, 0.0)";e.cellColorSource&&(s=this._compileChain(e.cellColorSource,"cellColor","vec4(0.0, 0.0, 0.0, 0.0)","v_uv","cellColor").colorVar);const o=(function(d,m,h){return d?`
	// Character output from generator
	vec4 charOutput = ${m};`:`
	// Derive character from color luminance
	float lum = _luminance(${h}.rgb);
	int charIdx = int(lum * 255.0);
	vec4 charOutput = vec4(float(charIdx % 256) / 255.0, float(charIdx / 256) / 255.0, 0.0, 0.0);`})(!!a,a??"vec4(0.0)",t.colorVar),c=this._feedbackTracker.getUsage();return{fragmentSource:(function(d){const{uniforms:m,glslFunctions:h,mainCode:v,charOutputCode:x,primaryColorVar:p,cellColorVar:y,charMapping:i,usesFeedback:u,usesCharFeedback:_,usesCellColorFeedback:C,externalLayers:f}=d,B=Array.from(m.values()).map(w=>`uniform ${w.type} ${w.name};`).join(`
...
`),V=[];if(f)for(const[,w]of f)w.usesChar&&V.push(`uniform sampler2D ${w.uniformPrefix}_char;`),w.usesPrimary&&V.push(`uniform sampler2D ${w.uniformPrefix}_primary;`),w.usesCellColor&&V.push(`uniform sampler2D ${w.uniformPrefix}_cell;`);return`#version 300 es
