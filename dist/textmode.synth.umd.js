(function(g,A){typeof exports=="object"&&typeof module<"u"?A(exports):typeof define=="function"&&define.amd?define(["exports"],A):A((g=typeof globalThis<"u"?globalThis:g||self).TextmodeSynth={})})(this,function(g){"use strict";const A={src:{returnType:"vec4",args:[{type:"vec2",name:"_st"}]},coord:{returnType:"vec2",args:[{type:"vec2",name:"_st"}]},color:{returnType:"vec4",args:[{type:"vec4",name:"_c0"}]},combine:{returnType:"vec4",args:[{type:"vec4",name:"_c0"},{type:"vec4",name:"_c1"}]},combineCoord:{returnType:"vec2",args:[{type:"vec2",name:"_st"},{type:"vec4",name:"_c0"}]}},z=new class{_transforms=new Map;_processedCache=new Map;register(e){this._transforms.has(e.name)&&console.warn(`[TransformRegistry] Overwriting existing transform: ${e.name}`),this._transforms.set(e.name,e),this._processedCache.delete(e.name)}registerMany(e){for(const t of e)this.register(t)}get(e){return this._transforms.get(e)}getProcessed(e){let t=this._processedCache.get(e);if(!t){const r=this._transforms.get(e);r&&(t=(function(a){const n=A[a.type],o=[...n.args,...a.inputs.map(c=>({type:c.type,name:c.name}))].map(c=>`${c.type} ${c.name}`).join(", "),s=`
${n.returnType} ${a.name}(${o}) {
${a.glsl}
}`;return{...a,glslFunction:s}})(r),this._processedCache.set(e,t))}return t}has(e){return this._transforms.has(e)}getByType(e){return Array.from(this._transforms.values()).filter(t=>t.type===e)}getNames(){return Array.from(this._transforms.keys())}getAll(){return Array.from(this._transforms.values())}getSourceTransforms(){return this.getByType("src")}remove(e){return this._processedCache.delete(e),this._transforms.delete(e)}clear(){this._transforms.clear(),this._processedCache.clear()}get size(){return this._transforms.size}},Y=new Set(["src"]),R=new class{_generatedFunctions={};_synthSourceClass=null;setSynthSourceClass(e){this._synthSourceClass=e}injectMethods(e){const t=z.getAll();for(const r of t)this._injectMethod(e,r)}_injectMethod(e,t){const{name:r,inputs:a,type:n}=t;e[r]=n==="combine"||n==="combineCoord"?function(o,...s){const c=a.map((i,u)=>s[u]??i.default);return this.addCombineTransform(r,o,c)}:function(...o){const s=a.map((c,i)=>o[i]??c.default);return this.addTransform(r,s)}}generateStandaloneFunctions(){if(!this._synthSourceClass)throw new Error("[TransformFactory] SynthSource class not set. Call setSynthSourceClass first.");const e={},t=z.getAll(),r=this._synthSourceClass;for(const a of t)if(Y.has(a.type)){const{name:n,inputs:o}=a;e[n]=(...s)=>{const c=new r,i=o.map((u,h)=>s[h]??u.default);return c.addTransform(n,i)}}return this._generatedFunctions=e,e}getGeneratedFunctions(){return this._generatedFunctions}addTransform(e,t){if(z.register(e),t&&this._injectMethod(t,e),Y.has(e.type)&&this._synthSourceClass){const r=this._synthSourceClass,{name:a,inputs:n}=e;this._generatedFunctions[a]=(...o)=>{const s=new r,c=n.map((i,u)=>o[u]??i.default);return s.addTransform(a,c)}}}},G=[{name:"osc",type:"src",inputs:[{name:"frequency",type:"float",default:60},{name:"sync",type:"float",default:.1},{name:"offset",type:"float",default:0}],glsl:`
	vec2 st = _st;
...
vec4 ${`src_${t}`}(vec2 _st) {
	return texture(${W[t]}, fract(_st));
}
`}}class J{_uniforms=new Map;_dynamicUpdaters=new Map;processArgument(t,r,a){if((function(n){return Array.isArray(n)&&n.length>0&&typeof n[0]=="number"})(t)){const n=`${a}_${r.name}`,o={name:n,type:r.type,value:r.default??0,isDynamic:!0},s=c=>(function(i,u){const h=i._speed??1,y=i._smooth??0;let x=u.time*h*(u.bpm/60)+(i._offset??0);if(y!==0){const C=i._ease??j.linear,_=x-y/2,v=i[Math.floor(B(_,i.length))],l=i[Math.floor(B(_+1,i.length))];return C(Math.min(B(_,1)/y,1))*(l-v)+v}return i[Math.floor(B(x,i.length))]})(t,c);return this._uniforms.set(n,o),this._dynamicUpdaters.set(n,s),{glslValue:n,uniform:o,updater:s}}if(typeof t=="function"){const n=`${a}_${r.name}`,o={name:n,type:r.type,value:r.default??0,isDynamic:!0};return this._uniforms.set(n,o),this._dynamicUpdaters.set(n,t),{glslValue:n,uniform:o,updater:t}}if(typeof t=="number")return{glslValue:b(t)};if(Array.isArray(t)&&typeof t[0]=="number"){const n=t;if(n.length===2)return{glslValue:`vec2(${b(n[0])}, ${b(n[1])})`};if(n.length===3)return{glslValue:`vec3(${b(n[0])}, ${b(n[1])}, ${b(n[2])})`};if(n.length===4)return{glslValue:`vec4(${b(n[0])}, ${b(n[1])}, ${b(n[2])}, ${b(n[3])})`}}return this.processDefault(r)}processDefault(t){const r=t.default;return typeof r=="number"?{glslValue:b(r)}:Array.isArray(r)?{glslValue:`vec${r.length}(${r.map(b).join(", ")})`}:{glslValue:"0.0"}}getUniforms(){return new Map(this._uniforms)}getDynamicUpdaters(){return new Map(this._dynamicUpdaters)}clear(){this._uniforms.clear(),this._dynamicUpdaters.clear()}}function b(e){const t=e.toString();return t.includes(".")?t:t+".0"}function X(e){return new Z().compile(e)}class Z{_uniformManager=new J;_feedbackTracker=new K;_externalLayerManager=new Q;_codeGenerator=new W;_glslFunctions=new Set;_mainCode=[];_varCounter=0;_currentTarget="main";_usesCharSource=!1;compile(t){this._reset();const r=this._compileChain(t,"main","vec4(1.0, 1.0, 1.0, 1.0)","v_uv","main");let a=r.charVar;t.charSource&&(a=this._compileCharSource(t));let n=r.colorVar;t.colorSource&&(n=this._compileChain(t.colorSource,"charColor","vec4(1.0, 1.0, 1.0, 1.0)","v_uv","charColor").colorVar);let o="vec4(0.0, 0.0, 0.0, 0.0)";t.cellColorSource&&(o=this._compileChain(t.cellColorSource,"cellColor","vec4(0.0, 0.0, 0.0, 0.0)","v_uv","cellColor").colorVar);const s=(function(u,h,y){return u?`
	// Character output from generator
	vec4 charOutput = ${h};`:`
	// Derive character from color luminance
	float lum = _luminance(${y}.rgb);
	int charIdx = int(lum * 255.0);
	vec4 charOutput = vec4(float(charIdx % 256) / 255.0, float(charIdx / 256) / 255.0, 0.0, 0.0);`})(!!a,a??"vec4(0.0)",r.colorVar),c=this._feedbackTracker.getUsage();return{fragmentSource:(function(u){const{uniforms:h,glslFunctions:y,mainCode:x,charOutputCode:C,primaryColorVar:_,cellColorVar:v,charMapping:l,usesFeedback:m,usesCharFeedback:p,usesCellColorFeedback:d,usesCharSource:f,externalLayers:k}=u,M=Array.from(h.values()).map(F=>`uniform ${F.type} ${F.name};`).join(`
`);let V="",$="";l&&(V=`uniform int u_charMap[${l.indices.length}];
uniform int u_charMapSize;`,$=`
	...
	charOutput.g = float(mappedCharIdx / 256) / 255.0;`);const P=[];m&&P.push("uniform sampler2D prevCharColorBuffer;"),p&&P.push("uniform sampler2D prevCharBuffer;"),d&&P.push("uniform sampler2D prevCellColorBuffer;");const oe=P.join(`
`),se=f?"uniform float u_charSourceCount;":"",U=[];if(k)for(const[,F]of k)F.usesChar&&U.push(`uniform sampler2D ${F.uniformPrefix}_char;`),F.usesCharColor&&U.push(`uniform sampler2D ${F.uniformPrefix}_primary;`),F.usesCellColor&&U.push(`uniform sampler2D ${F.uniformPrefix}_cell;`);return`#version 300 es
...
`})({uniforms:this._uniformManager.getUniforms(),glslFunctions:this._glslFunctions,mainCode:this._mainCode,charOutputCode:s,primaryColorVar:n,cellColorVar:o,charMapping:t.charMapping,usesFeedback:c.usesCharColorFeedback,usesCharFeedback:c.usesCharFeedback,usesCellColorFeedback:c.usesCellColorFeedback,usesCharSource:this._usesCharSource,externalLayers:this._externalLayerManager.getExternalLayers()}),uniforms:this._uniformManager.getUniforms(),dynamicUpdaters:this._uniformManager.getDynamicUpdaters(),charMapping:t.charMapping,usesCharColorFeedback:c.usesCharColorFeedback,usesCharFeedback:c.usesCharFeedback,usesCellColorFeedback:c.usesCellColorFeedback,usesCharSource:this._usesCharSource,externalLayers:this._externalLayerManager.getExternalLayers()}}_reset(){this._varCounter=0,this._uniformManager.clear(),this._feedbackTracker.reset(),this._externalLayerManager.reset(),this._glslFunctions.clear(),this._mainCode.length=0,this._currentTarget="main",this._usesCharSource=!1}_compileCharSource(t){this._usesCharSource=!0;const r=this._compileChain(t.charSource,"charSrc","vec4(1.0, 1.0, 1.0, 1.0)","v_uv","char"),a="charFromSource_"+this._varCounter++;return this._mainCode.push("	// Convert charSource color to character index"),this._mainCode.push(`	float charLum_${a} = _luminance(${r.colorVar}.rgb);`),this._mainCode.push(`	int charIdx_${a} = int(charLum_${a} * u_charSourceCount);`),this._mainCode.push(`	vec4 ${a} = vec4(float(charIdx_${a} % 256) / 255.0, float(charIdx_${a} / 256) / 255.0, 0.0, 0.0);`),a}_compileChain(t,r,a,n="v_uv",o="main"){const s=this._currentTarget;this._currentTarget=o;const c=`${r}_st`;let i,u,h,y=`${r}_c`;this._mainCode.push(`	vec2 ${c} = ${n};`),this._mainCode.push(`	vec4 ${y} = ${a};`);const x=t.transforms,C=x.map(l=>this._getProcessedTransform(l.name)),_=this._identifyCoordTransforms(C),v=l=>{const m=x[l],p=C[l];if(!p)return void console.warn(`[SynthCompiler] Unknown transform: ${m.name}`);const d=t.externalLayerRefs.get(l);m.name==="src"&&this._trackSrcUsage(d);const f=this._codeGenerator.getContextAwareGlslFunction(p,m.name,this._currentTarget,d,P=>this._externalLayerManager.getPrefix(P));this._glslFunctions.add(f);const k=this._processArguments(m.userArgs,p.inputs,`${r}_${l}_${m.name}`),M=t.nestedSources.get(l);let V;M&&(p.type==="combine"||p.type==="combineCoord")&&(V=this._compileChain(M,`${r}_nested_${l}`,a,c,o).colorVar);const $=this._codeGenerator.generateTransformCode(this._mainCode,p,this._varCounter++,c,y,i,u,h,k,this._currentTarget,V,d,P=>this._externalLayerManager.getPrefix(P));y=$.colorVar,$.charVar&&(i=$.charVar),$.flagsVar&&(u=$.flagsVar),$.rotationVar&&(h=$.rotationVar)};for(let l=_.length-1;l>=0;l--)v(_[l]);for(let l=0;l<x.length;l++){const m=C[l];(!m||m.type!=="coord"&&m.type!=="combineCoord")&&v(l)}return this._currentTarget=s,{coordVar:c,colorVar:y,charVar:i,flagsVar:u,rotationVar:h}}_identifyCoordTransforms(t){const r=[];for(let a=0;a<t.length;a++){const n=t[a];n&&(n.type!=="coord"&&n.type!=="combineCoord"||r.push(a))}return r}_trackSrcUsage(t){t?this._externalLayerManager.trackUsage(t,this._currentTarget):this._feedbackTracker.trackUsage(this._currentTarget)}_getProcessedTransform(t){return z.getProcessed(t)}_processArguments(t,r,a){const n=[];for(let o=0;o<r.length;o++){const s=r[o],c=t[o]??s.default,i=this._uniformManager.processArgument(c,s,a);n.push(i.glslValue)}return n}}class ee{_resolvedIndices;_lastFontCharacterCount=0;_lastChars="";resolve(t,r){const a=r.characters.length;if(this._resolvedIndices&&this._lastFontCharacterCount===a&&this._lastChars===t)return this._resolvedIndices;const n=Array.from(t),o=new Int32Array(n.length),s=r.characterMap,c=r.characters;for(let i=0;i<n.length;i++){const u=n[i],h=s.get(u);if(h!==void 0)o[i]=c.indexOf(h);else{const y=s.get(" ");o[i]=y!==void 0?c.indexOf(y):0}}return this._resolvedIndices=o,this._lastFontCharacterCount=a,this._lastChars=t,o}invalidate(){this._resolvedIndices=void 0,this._lastFontCharacterCount=0,this._lastChars=""}}function E(e={}){return{source:e.source??new S,compiled:e.compiled,shader:e.shader,characterResolver:e.characterResolver??new ee,needsCompile:e.needsCompile??!1,pingPongBuffers:e.pingPongBuffers,pingPongIndex:e.pingPongIndex??0,externalLayerMap:e.externalLayerMap,bpm:e.bpm}}let D=60;function te(e){e.bpm=function(t){return(function(r){D=r})(t),t}}function L(e){const t=new Map;for(const[,r]of e.externalLayerRefs)t.set(r.layerId,r.layer);for(const[,r]of e.nestedSources){const a=L(r);for(const[n,o]of a)t.set(n,o)}if(e.charSource){const r=L(e.charSource);for(const[a,n]of r)t.set(a,n)}if(e.colorSource){const r=L(e.colorSource);for(const[a,n]of r)t.set(a,n)}if(e.cellColorSource){const r=L(e.cellColorSource);for(const[a,n]of r)t.set(a,n)}return t}let O=null;function q(e,t,r){const a=r??O;if(a)try{a(e,t)}catch{}}function ne(e,t,r,a={}){let n;try{n=e()}catch(o){return q(o,t,a.onError),r}return(function(o){return o==null?!1:typeof o=="number"?Number.isFinite(o):Array.isArray(o)?o.length>0&&o.every(s=>typeof s=="number"&&Number.isFinite(s)):!1})(n)?n:(q(new Error(`[textmode.synth.js] Invalid dynamic parameter value for "${t}": ${N(n)}`),t,a.onError),r)}function N(e){if(e===void 0)return"undefined";if(e===null)return"null";if(typeof e=="number"){if(Number.isNaN(e))return"NaN";if(!Number.isFinite(e))return e>0?"Infinity":"-Infinity"}if(Array.isArray(e)){const t=e.findIndex(r=>typeof r!="number"||!Number.isFinite(r));if(t>=0)return`array with invalid element at index ${t}: ${N(e[t])}`}return String(e)}function re(e){const t=e.getPluginState(w);t&&(t.shader?.dispose&&t.shader.dispose(),t.pingPongBuffers&&(t.pingPongBuffers[0].dispose?.(),t.pingPongBuffers[1].dispose?.()))}const ae={name:w,version:"1.0.0",install(e,t){te(e),(function(r){r.extendLayer("synth",function(a){const n=this.grid!==void 0&&this.drawFramebuffer!==void 0;let o=this.getPluginState(w);o?(o.source=a,o.needsCompile=!0,o.characterResolver.invalidate(),n&&(o.compiled=X(a))):o=E({source:a,compiled:n?X(a):void 0,needsCompile:!0}),this.setPluginState(w,o)})})(t),(function(r){r.extendLayer("bpm",function(a){let n=this.getPluginState(w);n?n.bpm=a:n=E({bpm:a}),this.setPluginState(w,n)})})(t),(function(r){r.extendLayer("clearSynth",function(){const a=this.getPluginState(w);a&&(a.shader?.dispose&&a.shader.dispose(),a.pingPongBuffers&&(a.pingPongBuffers[0].dispose?.(),a.pingPongBuffers[1].dispose?.()),this.setPluginState(w,void 0))})})(t),t.registerLayerPreRenderHook(r=>(async function(a,n){const o=a.getPluginState(w);if(!o)return;const s=a.grid,c=a.drawFramebuffer;if(!s||!c)return;let i=!1;if(o.compiled||(o.compiled=X(o.source),o.externalLayerMap=L(o.source),i=!0,o.needsCompile=!0),o.needsCompile&&o.compiled&&(o.shader?.dispose&&o.shader.dispose(),i||(o.externalLayerMap=L(o.source)),o.shader=await n.createFilterShader(o.compiled.fragmentSource),o.needsCompile=!1),!o.shader||!o.compiled)return;const u=o.compiled.usesCharColorFeedback,h=o.compiled.usesCharFeedback,y=o.compiled.usesCellColorFeedback,x=u||h||y;x&&!o.pingPongBuffers&&(o.pingPongBuffers=[n.createFramebuffer({width:s.cols,height:s.rows,attachments:3}),n.createFramebuffer({width:s.cols,height:s.rows,attachments:3})],o.pingPongIndex=0);const C={time:n.secs,frameCount:n.frameCount,width:s.width,height:s.height,cols:s.cols,rows:s.rows,bpm:o.bpm??D},_=new Map;for(const[l,m]of o.compiled.dynamicUpdaters){const p=o.compiled.uniforms.get(l),d=ne(()=>m(C),l,p?.value??0,{onError:o.onDynamicError});_.set(l,d)}const v=l=>{n.setUniform("time",n.frameCount/n.targetFrameRate()),n.setUniform("resolution",[C.cols,C.rows]);for(const[p,d]of _)n.setUniform(p,d);for(const[p,d]of o.compiled.uniforms)d.isDynamic||typeof d.value=="function"||n.setUniform(p,d.value);if(o.compiled.charMapping){const p=o.characterResolver.resolve(o.compiled.charMapping.chars,a.font);n.setUniform("u_charMap",p),n.setUniform("u_charMapSize",p.length)}if(o.compiled.usesCharSource){const p=o.compiled.charMapping?o.compiled.charMapping.chars.length:a.font.characters.length;n.setUniform("u_charSourceCount",p)}l&&(u&&n.setUniform("prevCharColorBuffer",l.textures[1]),h&&n.setUniform("prevCharBuffer",l.textures[0]),y&&n.setUniform("prevCellColorBuffer",l.textures[2]));const m=o.compiled.externalLayers;if(m&&m.size>0&&o.externalLayerMap)for(const[p,d]of m){const f=o.externalLayerMap.get(p);if(!f){console.warn(`[textmode.synth.js] External layer not found: ${p}`);continue}const k=f.getPluginState(w);let M;k?.pingPongBuffers?M=k.pingPongBuffers[k.pingPongIndex].textures:f.drawFramebuffer&&(M=f.drawFramebuffer.textures),M&&(d.usesChar&&n.setUniform(`${d.uniformPrefix}_char`,M[0]),d.usesCharColor&&n.setUniform(`${d.uniformPrefix}_primary`,M[1]),d.usesCellColor&&n.setUniform(`${d.uniformPrefix}_cell`,M[2]))}};if(x&&o.pingPongBuffers){const l=o.pingPongBuffers[o.pingPongIndex],m=o.pingPongBuffers[1-o.pingPongIndex];m.begin(),n.clear(),n.shader(o.shader),v(l),n.rect(s.cols,s.rows),m.end(),c.begin(),n.clear(),n.shader(o.shader),v(l),n.rect(s.cols,s.rows),c.end(),o.pingPongIndex=1-o.pingPongIndex}else c.begin(),n.clear(),n.shader(o.shader),v(null),n.rect(s.cols,s.rows),c.end()})(r,e)),t.registerLayerDisposedHook(re)},uninstall(e,t){const r=[t.layerManager.base,...t.layerManager.all];for(const a of r){const n=a.getPluginState(w);n&&(n.shader?.dispose&&n.shader.dispose(),n.pingPongBuffers&&(n.pingPongBuffers[0].dispose?.(),n.pingPongBuffers[1].dispose?.()))}delete e.bpm,t.removeLayerExtension("synth"),t.removeLayerExtension("bpm"),t.removeLayerExtension("clearSynth")}};g.SynthPlugin=ae,g.SynthSource=S,g.cellColor=e=>{const t=new S;return t._colorSource=e,t},g.char=(e,t)=>{const r=new S;return r._charSource=e,r._charCount=t,r},g.charColor=e=>{const t=new S;return t._colorSource=e,t},g.gradient=function(e){return T.gradient(e)},g.noise=function(e,t){return T.noise(e,t)},g.osc=function(e,t,r){return T.osc(e,t,r)},g.paint=e=>{const t=new S;return t._colorSource=e,t._cellColorSource=e,t},g.setGlobalErrorCallback=function(e){O=e},g.shape=function(e,t,r){return T.shape(e,t,r)},g.solid=function(e,t,r,a){return T.solid(e,t,r,a)},g.src=e=>{const t=T.src;if(!e)return t();const r=new S,a=e.id??`layer_${Date.now()}_${Math.random().toString(36).slice(2,9)}`;return r.addExternalLayerRef({layerId:a,layer:e}),r},g.voronoi=function(e,t,r){return T.voronoi(e,t,r)},Object.defineProperty(g,Symbol.toStringTag,{value:"Module"})});
