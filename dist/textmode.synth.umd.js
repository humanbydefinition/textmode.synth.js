...
uniform int u_charMapSize;`,z=`
	...
	charOutput.g = float(mappedCharIdx / 256) / 255.0;`);const S=[];p&&S.push("uniform sampler2D prevCharColorBuffer;"),d&&S.push("uniform sampler2D prevCharBuffer;"),b&&S.push("uniform sampler2D prevCellColorBuffer;");const V=S.join(`
`),U=[];if(m)for(const[,F]of m)F.usesChar&&U.push(`uniform sampler2D ${F.uniformPrefix}_char;`),F.usesCharColor&&U.push(`uniform sampler2D ${F.uniformPrefix}_primary;`),F.usesCellColor&&U.push(`uniform sampler2D ${F.uniformPrefix}_cell;`);return`#version 300 es
