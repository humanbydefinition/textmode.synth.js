(function(g,I){typeof exports=="object"&&typeof module<"u"?I(exports):typeof define=="function"&&define.amd?define(["exports"],I):I((g=typeof globalThis<"u"?globalThis:g||self).TextmodeSynth={})})(this,function(g){"use strict";const I={src:{returnType:"vec4",args:[{type:"vec2",name:"_st"}]},coord:{returnType:"vec2",args:[{type:"vec2",name:"_st"}]},color:{returnType:"vec4",args:[{type:"vec4",name:"_c0"}]},combine:{returnType:"vec4",args:[{type:"vec4",name:"_c0"},{type:"vec4",name:"_c1"}]},combineCoord:{returnType:"vec2",args:[{type:"vec2",name:"_st"},{type:"vec4",name:"_c0"}]}},T=new class{_transforms=new Map;_processedCache=new Map;register(t){this._transforms.has(t.name)&&console.warn(`[TransformRegistry] Overwriting existing transform: ${t.name}`),this._transforms.set(t.name,t),this._processedCache.delete(t.name)}registerMany(t){for(const e of t)this.register(e)}get(t){return this._transforms.get(t)}getProcessed(t){let e=this._processedCache.get(t);if(!e){const r=this._transforms.get(t);r&&(e=(function(a){const n=I[a.type],o=[...n.args,...a.inputs.map(s=>({type:s.type,name:s.name}))].map(s=>`${s.type} ${s.name}`).join(", "),c=`
${n.returnType} ${a.name}(${o}) {
${a.glsl}
}`;return{...a,glslFunction:c}})(r),this._processedCache.set(t,e))}return e}has(t){return this._transforms.has(t)}getByType(t){return Array.from(this._transforms.values()).filter(e=>e.type===t)}getNames(){return Array.from(this._transforms.keys())}getAll(){return Array.from(this._transforms.values())}getSourceTransforms(){return this.getByType("src")}remove(t){return this._processedCache.delete(t),this._transforms.delete(t)}clear(){this._transforms.clear(),this._processedCache.clear()}get size(){return this._transforms.size}},D=new Set(["src"]),R=new class{_generatedFunctions={};_synthSourceClass=null;setSynthSourceClass(t){this._synthSourceClass=t}injectMethods(t){const e=T.getAll();for(const r of e)this._injectMethod(t,r)}_injectMethod(t,e){const{name:r,inputs:a,type:n}=e;t[r]=n==="combine"||n==="combineCoord"?function(o,...c){const s=a.map((i,u)=>c[u]??i.default);return this.addCombineTransform(r,o,s)}:function(...o){const c=a.map((s,i)=>o[i]??s.default);return this.addTransform(r,c)}}generateStandaloneFunctions(){if(!this._synthSourceClass)throw new Error("[TransformFactory] SynthSource class not set. Call setSynthSourceClass first.");const t={},e=T.getAll(),r=this._synthSourceClass;for(const a of e)if(D.has(a.type)){const{name:n,inputs:o}=a;t[n]=(...c)=>{const s=new r,i=o.map((u,d)=>c[d]??u.default);return s.addTransform(n,i)}}return this._generatedFunctions=t,t}getGeneratedFunctions(){return this._generatedFunctions}addTransform(t,e){if(T.register(t),e&&this._injectMethod(e,t),D.has(t.type)&&this._synthSourceClass){const r=this._synthSourceClass,{name:a,inputs:n}=t;this._generatedFunctions[a]=(...o)=>{const c=new r,s=n.map((i,u)=>o[u]??i.default);return c.addTransform(a,s)}}}},H=[{name:"osc",type:"src",inputs:[{name:"frequency",type:"float",default:60},{name:"sync",type:"float",default:.1},{name:"offset",type:"float",default:0}],glsl:`
	vec2 st = _st;
...
vec4 ${`src_${e}`}(vec2 _st) {
	return texture(${Z[e]}, fract(_st));
}
`}}class te{_uniforms=new Map;_dynamicUpdaters=new Map;processArgument(e,r,a){if((function(n){return Array.isArray(n)&&n.length>0&&typeof n[0]=="number"})(e)){const n=`${a}_${r.name}`,o={name:n,type:r.type,value:r.default??0,isDynamic:!0},c=s=>(function(i,u){const d=i._speed??1,h=i._smooth??0;let _=u.time*d*(u.bpm/60)+(i._offset??0);if(h!==0){const x=i._ease??j.linear,m=_-h/2,y=i[Math.floor(U(m,i.length))],l=i[Math.floor(U(m+1,i.length))];return x(Math.min(U(m,1)/h,1))*(l-y)+y}return i[Math.floor(U(_,i.length))]})(e,s);return this._uniforms.set(n,o),this._dynamicUpdaters.set(n,c),{glslValue:n,uniform:o,updater:c}}if(typeof e=="function"){const n=`${a}_${r.name}`,o={name:n,type:r.type,value:r.default??0,isDynamic:!0};return this._uniforms.set(n,o),this._dynamicUpdaters.set(n,e),{glslValue:n,uniform:o,updater:e}}if(typeof e=="number")return{glslValue:b(e)};if(Array.isArray(e)&&typeof e[0]=="number"){const n=e;if(n.length===2)return{glslValue:`vec2(${b(n[0])}, ${b(n[1])})`};if(n.length===3)return{glslValue:`vec3(${b(n[0])}, ${b(n[1])}, ${b(n[2])})`};if(n.length===4)return{glslValue:`vec4(${b(n[0])}, ${b(n[1])}, ${b(n[2])}, ${b(n[3])})`}}return this.processDefault(r)}processDefault(e){const r=e.default;return typeof r=="number"?{glslValue:b(r)}:Array.isArray(r)?{glslValue:`vec${r.length}(${r.map(b).join(", ")})`}:{glslValue:"0.0"}}getUniforms(){return new Map(this._uniforms)}getDynamicUpdaters(){return new Map(this._dynamicUpdaters)}clear(){this._uniforms.clear(),this._dynamicUpdaters.clear()}}function b(t){const e=t.toString();return e.includes(".")?e:e+".0"}function X(t){return new ne().compile(t)}class ne{_uniformManager=new te;_feedbackTracker=new W;_externalLayerManager=new J;_codeGenerator=new ee;_glslFunctions=new Set;_mainCode=[];_varCounter=0;_currentTarget="main";compile(e){this._reset();const r=this._compileChain(e,"main","vec4(1.0, 1.0, 1.0, 1.0)","v_uv","main");let a=r.charVar;e.charSource&&(a=this._compileCharSource(e));let n=r.colorVar;e.colorSource&&(n=this._compileChain(e.colorSource,"charColor","vec4(1.0, 1.0, 1.0, 1.0)","v_uv","charColor").colorVar);let o="vec4(0.0, 0.0, 0.0, 0.0)";e.cellColorSource&&(o=this._compileChain(e.cellColorSource,"cellColor","vec4(0.0, 0.0, 0.0, 0.0)","v_uv","cellColor").colorVar);const c=(function(u,d,h){return u?`
	// Character output from generator
	vec4 charOutput = ${d};`:`
	// Derive character from color luminance
	float lum = _luminance(${h}.rgb);
	int charIdx = int(lum * 255.0);
	vec4 charOutput = vec4(float(charIdx % 256) / 255.0, float(charIdx / 256) / 255.0, 0.0, 0.0);`})(!!a,a??"vec4(0.0)",r.colorVar),s=this._feedbackTracker.getUsage();return{fragmentSource:(function(u){const{uniforms:d,glslFunctions:h,mainCode:_,charOutputCode:x,primaryColorVar:m,cellColorVar:y,charMapping:l,usesFeedback:p,usesCharFeedback:v,usesCellColorFeedback:C,externalLayers:f}=u,E=Array.from(d.values()).map($=>`uniform ${$.type} ${$.name};`).join(`
`);let L="",A="";l&&(L=`uniform int u_charMap[${l.indices.length}];
uniform int u_charMapSize;`,A=`
	...
	charOutput.g = float(mappedCharIdx / 256) / 255.0;`);const S=[];p&&S.push("uniform sampler2D prevCharColorBuffer;"),v&&S.push("uniform sampler2D prevCharBuffer;"),C&&S.push("uniform sampler2D prevCellColorBuffer;");const z=S.join(`
`),V=[];if(f)for(const[,$]of f)$.usesChar&&V.push(`uniform sampler2D ${$.uniformPrefix}_char;`),$.usesCharColor&&V.push(`uniform sampler2D ${$.uniformPrefix}_primary;`),$.usesCellColor&&V.push(`uniform sampler2D ${$.uniformPrefix}_cell;`);return`#version 300 es
...
`})({uniforms:this._uniformManager.getUniforms(),glslFunctions:this._glslFunctions,mainCode:this._mainCode,charOutputCode:c,primaryColorVar:n,cellColorVar:o,charMapping:e.charMapping,usesFeedback:s.usesCharColorFeedback,usesCharFeedback:s.usesCharFeedback,usesCellColorFeedback:s.usesCellColorFeedback,externalLayers:this._externalLayerManager.getExternalLayers()}),uniforms:this._uniformManager.getUniforms(),dynamicUpdaters:this._uniformManager.getDynamicUpdaters(),charMapping:e.charMapping,usesCharColorFeedback:s.usesCharColorFeedback,usesCharFeedback:s.usesCharFeedback,usesCellColorFeedback:s.usesCellColorFeedback,externalLayers:this._externalLayerManager.getExternalLayers()}}_reset(){this._varCounter=0,this._uniformManager.clear(),this._feedbackTracker.reset(),this._externalLayerManager.reset(),this._glslFunctions.clear(),this._mainCode.length=0,this._currentTarget="main"}_compileCharSource(e){const r=this._compileChain(e.charSource,"charSrc","vec4(1.0, 1.0, 1.0, 1.0)","v_uv","char"),a="charFromSource_"+this._varCounter++,n=e.charCount??256;return this._mainCode.push("	// Convert charSource color to character index"),this._mainCode.push(`	float charLum_${a} = _luminance(${r.colorVar}.rgb);`),this._mainCode.push(`	int charIdx_${a} = int(charLum_${a} * ${n.toFixed(1)});`),this._mainCode.push(`	vec4 ${a} = vec4(float(charIdx_${a} % 256) / 255.0, float(charIdx_${a} / 256) / 255.0, 0.0, 0.0);`),a}_compileChain(e,r,a,n="v_uv",o="main"){const c=this._currentTarget;this._currentTarget=o;const s=`${r}_st`;let i,u,d,h=`${r}_c`;this._mainCode.push(`	vec2 ${s} = ${n};`),this._mainCode.push(`	vec4 ${h} = ${a};`);const _=e.transforms,x=_.map(l=>this._getProcessedTransform(l.name)),m=this._identifyCoordTransforms(x),y=l=>{const p=_[l],v=x[l];if(!v)return void console.warn(`[SynthCompiler] Unknown transform: ${p.name}`);const C=e.externalLayerRefs.get(l);p.name==="src"&&this._trackSrcUsage(C);const f=this._codeGenerator.getContextAwareGlslFunction(v,p.name,this._currentTarget,C,z=>this._externalLayerManager.getPrefix(z));this._glslFunctions.add(f);const E=this._processArguments(p.userArgs,v.inputs,`${r}_${l}_${p.name}`),L=e.nestedSources.get(l);let A;L&&(v.type==="combine"||v.type==="combineCoord")&&(A=this._compileChain(L,`${r}_nested_${l}`,a,s,o).colorVar);const S=this._codeGenerator.generateTransformCode(this._mainCode,v,this._varCounter++,s,h,i,u,d,E,this._currentTarget,A,C,z=>this._externalLayerManager.getPrefix(z));h=S.colorVar,S.charVar&&(i=S.charVar),S.flagsVar&&(u=S.flagsVar),S.rotationVar&&(d=S.rotationVar)};for(let l=m.length-1;l>=0;l--)y(m[l]);for(let l=0;l<_.length;l++){const p=x[l];(!p||p.type!=="coord"&&p.type!=="combineCoord")&&y(l)}return this._currentTarget=c,{coordVar:s,colorVar:h,charVar:i,flagsVar:u,rotationVar:d}}_identifyCoordTransforms(e){const r=[];for(let a=0;a<e.length;a++){const n=e[a];n&&(n.type!=="coord"&&n.type!=="combineCoord"||r.push(a))}return r}_trackSrcUsage(e){e?this._externalLayerManager.trackUsage(e,this._currentTarget):this._feedbackTracker.trackUsage(this._currentTarget)}_getProcessedTransform(e){return T.getProcessed(e)}_processArguments(e,r,a){const n=[];for(let o=0;o<r.length;o++){const c=r[o],s=e[o]??c.default,i=this._uniformManager.processArgument(s,c,a);n.push(i.glslValue)}return n}}class B{static _fontIndexCache=new WeakMap;_resolvedIndices;_lastFont;_lastChars="";resolve(e,r){if(this._resolvedIndices&&this._lastFont===r&&this._lastChars===e)return this._resolvedIndices;let a=B._fontIndexCache.get(r);if(!a){a=new Map;const u=r.characters;for(let d=0;d<u.length;d++)a.set(u[d],d);B._fontIndexCache.set(r,a)}const n=Array.from(e),o=new Int32Array(n.length),c=r.characterMap,s=c.get(" "),i=s&&a.has(s)?a.get(s):0;for(let u=0;u<n.length;u++){const d=n[u],h=c.get(d);if(h){const _=a.get(h);o[u]=_!==void 0?_:i}else o[u]=i}return this._resolvedIndices=o,this._lastFont=r,this._lastChars=e,o}invalidate(){this._resolvedIndices=void 0,this._lastFont=void 0,this._lastChars=""}}function O(t={}){return{source:t.source??new w,compiled:t.compiled,shader:t.shader,characterResolver:t.characterResolver??new B,needsCompile:t.needsCompile??!1,pingPongBuffers:t.pingPongBuffers,pingPongIndex:t.pingPongIndex??0,externalLayerMap:t.externalLayerMap,bpm:t.bpm}}let q=60;function re(t){t.bpm=function(e){return(function(r){q=r})(e),e}}function P(t){const e=new Map;for(const[,r]of t.externalLayerRefs)e.set(r.layerId,r.layer);for(const[,r]of t.nestedSources){const a=P(r);for(const[n,o]of a)e.set(n,o)}if(t.charSource){const r=P(t.charSource);for(const[a,n]of r)e.set(a,n)}if(t.colorSource){const r=P(t.colorSource);for(const[a,n]of r)e.set(a,n)}if(t.cellColorSource){const r=P(t.cellColorSource);for(const[a,n]of r)e.set(a,n)}return e}let N=null;const Y=new Map,G=new Map;function K(t,e,r){const a=r??N;if(a)try{a(t,e)}catch{}else{const n=Date.now();if(n-(G.get(e)??0)>=1e3){G.set(e,n);const o=t instanceof Error?t.message:String(t);console.warn(`[textmode.synth.js] Dynamic parameter error in "${e}": ${o}`)}}}function ae(t,e,r){try{const a=t();if(!(function(n){return n==null?!1:typeof n=="number"?Number.isFinite(n):Array.isArray(n)?n.length>0&&n.every(o=>typeof o=="number"&&Number.isFinite(o)):!1})(a)){K(new Error(`Invalid dynamic parameter value: ${Q(a)}`),e,r.onError);const n=Y.get(e);return n!==void 0?n:r.fallback}return Y.set(e,a),a}catch(a){K(a,e,r.onError);const n=Y.get(e);return n!==void 0?n:r.fallback}}function Q(t){if(t===void 0)return"undefined";if(t===null)return"null";if(typeof t=="number"){if(Number.isNaN(t))return"NaN";if(!Number.isFinite(t))return t>0?"Infinity":"-Infinity"}if(Array.isArray(t)){const e=t.findIndex(r=>typeof r!="number"||!Number.isFinite(r));if(e>=0)return`array with invalid element at index ${e}: ${Q(t[e])}`}return String(t)}function oe(t){const e=t.getPluginState(M);e&&(e.shader?.dispose&&e.shader.dispose(),e.pingPongBuffers&&(e.pingPongBuffers[0].dispose?.(),e.pingPongBuffers[1].dispose?.()))}const se={name:M,version:"1.0.0",install(t,e){re(t),(function(r){r.extendLayer("synth",function(a){const n=this.grid!==void 0&&this.drawFramebuffer!==void 0;let o=this.getPluginState(M);o?(o.source=a,o.needsCompile=!0,o.characterResolver.invalidate(),n&&(o.compiled=X(a))):o=O({source:a,compiled:n?X(a):void 0,needsCompile:!0}),this.setPluginState(M,o)})})(e),(function(r){r.extendLayer("bpm",function(a){let n=this.getPluginState(M);n?n.bpm=a:n=O({bpm:a}),this.setPluginState(M,n)})})(e),(function(r){r.extendLayer("clearSynth",function(){const a=this.getPluginState(M);a&&(a.shader?.dispose&&a.shader.dispose(),a.pingPongBuffers&&(a.pingPongBuffers[0].dispose?.(),a.pingPongBuffers[1].dispose?.()),this.setPluginState(M,void 0))})})(e),e.registerLayerPreRenderHook(r=>(async function(a,n){const o=a.getPluginState(M);if(!o)return;const c=a.grid,s=a.drawFramebuffer;if(!c||!s||(o.compiled||(o.compiled=X(o.source),o.externalLayerMap=P(o.source),o.needsCompile=!0),o.needsCompile&&o.compiled&&(o.shader?.dispose&&o.shader.dispose(),o.externalLayerMap=P(o.source),o.shader=await n.createFilterShader(o.compiled.fragmentSource),o.needsCompile=!1),!o.shader||!o.compiled))return;const i=o.compiled.usesCharColorFeedback,u=o.compiled.usesCharFeedback,d=o.compiled.usesCellColorFeedback,h=i||u||d;h&&!o.pingPongBuffers&&(o.pingPongBuffers=[n.createFramebuffer({width:c.cols,height:c.rows,attachments:3}),n.createFramebuffer({width:c.cols,height:c.rows,attachments:3})],o.pingPongIndex=0);const _={time:n.secs,frameCount:n.frameCount,width:c.width,height:c.height,cols:c.cols,rows:c.rows,bpm:o.bpm??q},x=m=>{n.setUniform("time",n.secs),n.setUniform("resolution",[_.cols,_.rows]);for(const[l,p]of o.compiled.dynamicUpdaters){const v=o.compiled.uniforms.get(l),C=ae(()=>p(_),l,{fallback:v?.value??0,onError:o.onDynamicError});n.setUniform(l,C)}for(const[l,p]of o.compiled.uniforms)p.isDynamic||typeof p.value=="function"||n.setUniform(l,p.value);if(o.compiled.charMapping){const l=o.characterResolver.resolve(o.compiled.charMapping.chars,a.font);n.setUniform("u_charMap",l),n.setUniform("u_charMapSize",l.length)}m&&(i&&n.setUniform("prevCharColorBuffer",m.textures[1]),u&&n.setUniform("prevCharBuffer",m.textures[0]),d&&n.setUniform("prevCellColorBuffer",m.textures[2]));const y=o.compiled.externalLayers;if(y&&y.size>0&&o.externalLayerMap)for(const[l,p]of y){const v=o.externalLayerMap.get(l);if(!v){console.warn(`[textmode.synth.js] External layer not found: ${l}`);continue}const C=v.getPluginState(M);let f;C?.pingPongBuffers?f=C.pingPongBuffers[C.pingPongIndex].textures:v.drawFramebuffer&&(f=v.drawFramebuffer.textures),f&&(p.usesChar&&n.setUniform(`${p.uniformPrefix}_char`,f[0]),p.usesCharColor&&n.setUniform(`${p.uniformPrefix}_primary`,f[1]),p.usesCellColor&&n.setUniform(`${p.uniformPrefix}_cell`,f[2]))}};if(h&&o.pingPongBuffers){const m=o.pingPongBuffers[o.pingPongIndex],y=o.pingPongBuffers[1-o.pingPongIndex];y.begin(),n.clear(),n.shader(o.shader),x(m),n.rect(c.cols,c.rows),y.end(),s.begin(),n.clear(),n.shader(o.shader),x(m),n.rect(c.cols,c.rows),s.end(),o.pingPongIndex=1-o.pingPongIndex}else s.begin(),n.clear(),n.shader(o.shader),x(null),n.rect(c.cols,c.rows),s.end()})(r,t)),e.registerLayerDisposedHook(oe)},uninstall(t,e){const r=[e.layerManager.base,...e.layerManager.all];for(const a of r){const n=a.getPluginState(M);n&&(n.shader?.dispose&&n.shader.dispose(),n.pingPongBuffers&&(n.pingPongBuffers[0].dispose?.(),n.pingPongBuffers[1].dispose?.()))}delete t.bpm,e.removeLayerExtension("synth"),e.removeLayerExtension("bpm"),e.removeLayerExtension("clearSynth")}};g.SynthPlugin=se,g.SynthSource=w,g.cellColor=t=>{const e=new w;return e._colorSource=t,e},g.char=(t,e)=>{const r=new w;return r._charSource=t,r._charCount=e,r},g.charColor=t=>{const e=new w;return e._colorSource=t,e},g.gradient=function(t){return F.gradient(t)},g.noise=function(t,e){return F.noise(t,e)},g.osc=function(t,e,r){return F.osc(t,e,r)},g.paint=t=>{const e=new w;return e._colorSource=t,e._cellColorSource=t,e},g.setGlobalErrorCallback=function(t){N=t},g.shape=function(t,e,r){return F.shape(t,e,r)},g.solid=function(t,e,r,a){return F.solid(t,e,r,a)},g.src=t=>{const e=F.src;if(!t)return e();const r=new w,a=t.id??`layer_${Date.now()}_${Math.random().toString(36).slice(2,9)}`;return r.addExternalLayerRef({layerId:a,layer:t}),r},g.voronoi=function(t,e,r){return F.voronoi(t,e,r)},Object.defineProperty(g,Symbol.toStringTag,{value:"Module"})});
