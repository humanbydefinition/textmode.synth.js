...
import { extendTextModeBpm } from './extensions/extendTextModeBpm';
import { extendLayerSynth } from './extensions/extendLayerSynth';
import { extendLayerBpm } from './extensions/extendLayerBpm';
import { extendLayerClearSynth } from './extensions/extendLayerClearSynth';
import { useSynthRender } from './hooks/useSynthRender';
import { useSynthDispose } from './hooks/useSynthDispose';
...
export const SynthPlugin: TextmodePlugin = {
	...
	install(textmodifier, api: TextmodePluginAPI) {
		// Extensions
		extendTextModeBpm(textmodifier);
		extendLayerSynth(api);
		extendLayerBpm(api);
		extendLayerClearSynth(api);

		// Hooks
		api.registerLayerPreRenderHook((layer) => useSynthRender(layer, textmodifier));
		api.registerLayerDisposedHook(useSynthDispose);
	},
